...
const EmailSignupForm = ({ id, align = "center", onRequestSignup }: EmailSignupFormProps) => {
  ...
  return (
    <Form id={id} className={`gap-cg-0 w-full max-w-[560px] ${align === "left" ? "ml-0 mr-auto" : "mx-auto"} [scroll-margin-top:calc(var(--landing-header-height,_calc(var(--spacing-cg-1)_*_28))_+_var(--spacing-cg-6))]`} onSubmit={async (event) => {
      ...
      >
        <TextField name="email" type="email" autoComplete="email" placeholder="이메일 주소를 입력해주세요…" leadingIcon={<Mail size={20} aria-hidden="true" />} wrapperClassName="[&&]:rounded-cg-full [&&]:bg-cg-border [&&]:text-cg-ink [&&]:shadow-[inset_4px_4px_8px_rgba(201,208,217,0.8),inset_-4px_-4px_8px_rgba(255,255,255,0.6)] [&&:not(:has(input[aria-invalid=true]))]:focus-within:border-cg-border [&&:not(:has(input[aria-invalid=true]))]:focus-within:ring-3 [&&:not(:has(input[aria-invalid=true]))]:focus-within:ring-[var(--landing-accent)]/45 motion-reduce:transition-none" trailingAction={<Button type="submit" size="sm" className="[&&]:hover:bg-cg-action [&&]:hover:text-cg-ink motion-safe:hover:-translate-y-px hover:shadow-[0_8px_18px_rgba(255,200,87,0.42)]" loading={submitting} trailingIcon={<Bell size={16} />}>알림 받기</Button>} />
      </Form.Field>
